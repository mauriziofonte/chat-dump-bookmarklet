...
const { initUI, showError, showExportOptions, showLoading } = await import('../src/UIManager.js')
...
showLoading()
console.log('loading toast with spinner:', !!document.querySelector('.chatdump-toast .toast-spinner'))
showError('Test error')
console.log('error toast:', document.querySelector('.chatdump-toast').textContent.includes('Test error'))
showExportOptions({ mdText: '# md', htmlText: '<h1>html</h1>', txtText: 'txt', filename: 'test-file' })
const links = [...document.querySelectorAll('.chatdump-toast a')]
console.log('toast replaced, 6 links:', document.querySelectorAll('.chatdump-toast').length === 1 && links.length === 6)
console.log(
	'download names ok:',
	links[0].getAttribute('download') === 'test-file.md' &&
		links[1].getAttribute('download') === 'test-file.html' &&
		links[2].getAttribute('download') === 'test-file.txt',
)
